...
class Admin extends Base
{
    ...
    public function add(Request $request)
    {
        if ($request->isPost()) {
            ...
            $adminCode = \app\admin\model\Admin::where('admin_code', $admin_code)->find();
            if ($adminCode) {
        ...
        $roleList = \app\admin\model\Role::order('role_id desc')->select();
    ...
    public function edit(Request $request)
    {
        if ($request->isPost()) {
            ...
            $adminCode = \app\admin\model\Admin::where([['admin_code', '=', $admin_code], ['admin_id', '<>', $admin_id]])->find();
            if ($adminCode) {
                echo $this->errorJson(0, "成员账号[$admin_code]已经存在");
                return;
            }
        ...
        $admin = \app\admin\model\Admin::where('admin_id', $admin_id)->find();
    ...
    public function delete(Request $request)
    {
        if ($request->isPost()) {
            ...
            $affected = \app\admin\model\Admin::where('admin_id', $admin_id)->delete();
            if ($affected > 0) {
                echo $this->successJson();
                exit;
            } else {
    ...
    public function updateuserpwd(Request $request)
    {
        if ($request->isPost()) {
            ...
            $adminUser = \app\admin\model\Admin::where('admin_id', $admin_id)->find();
            if (!$adminUser) {
