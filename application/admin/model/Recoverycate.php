...
class Recoverycate extends Model
{
    //设置数据表名
    protected $table = 'mrs_recovery_cate';
    ...
    public function getParentCate($parent_id = 0)
    {
        ...
        $firstCateList = \app\admin\model\Recoverycate::where('cate_level', '=', 1)->order('order_no', 'asc')->select();
        $secondCateList = \app\admin\model\Recoverycate::where('cate_level', '=', 2)->order('order_no', 'asc')->select();
        $threeCateList = \app\admin\model\Recoverycate::where('cate_level', '=', 3)->order('order_no', 'asc')->select();
        ...
        foreach ($firstCateList as $k1 => $v1) {
            ...
            foreach ($secondCateList as $k2 => $v2) {
                if ($v1['cate_id'] == $v2['parent_id']) {
                    ...
                    foreach ($threeCateList as $k3 => $v3) {
                        if ($v2['cate_id'] == $v3['parent_id']) {
                            $cateData[] = ['cate_id' => $v3['cate_id'], 'cate_name' => '  -  　' . $v3['cate_name']];
                        }
                    }
                }
            }
